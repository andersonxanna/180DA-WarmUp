...
def on_message(client, userdata, message):
    ...
    if str(message.payload)[0:3] == "b'2":
        print("PLAYER 2", str(message.payload))
        p2 = True
        p2_move = str(message.payload)[3]
...
while running: # perhaps add a stopping condition using some break or something
    ...
    if not started:
        ...
        if result:
            text = font.render('YOU <-- Game over! You {} --> Other Player'.format(result_char), True, blue, (255,255,255))
            change_text(text, textRect, pygame, imgs[char_map[p1_move]], imgs[char_map[p2_move]])
            result = False
        elif p1 and not p2:
            text = font.render('YOU <-- Rock paper scissors --> Other Player', True, blue, (255,255,255))
